...
- `install.sh` takes no args; non-interactive identity via `CHEZMOI_NAME`/`CHEZMOI_EMAIL`, else it prompts once and stores `~/.config/chezmoi/chezmoi.toml`. Ends with `chezmoi apply --source dotfiles/` + `reports/versions.sh`.
